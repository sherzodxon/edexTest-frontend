...
export default function StudentTestPage() {
    const {id} = useParams();
    const [test,
        setTest] = useState < TestData | null > (null);
    const [answers,
            setAnswers] = useState < Record < number,
        string >> ({});
    const [timeLeft,
        setTimeLeft] = useState < number > (0);
    const [isSubmitting,
        setIsSubmitting] = useState(false);
    const [fullImage,
        setFullImage] = useState < string | null > (null);
    ...
    useEffect(() => {
        ...
        const fetchData = async() => {
            ...
            if (now < start) {
                toast("Test hali boshlanmagan. Iltimos, boshlanish vaqtini kuting.", {duration: 6000});
    ...
    const handleSubmit = async() => {
        ...
        try {
            ...
            for (const q of test.questions) {
                ...
                const foundOpt = q
                    .options
                    .find((o) => String(o.text).trim() === String(selectedText).trim());

                if (foundOpt) 
                    answerArray.push({questionId: q.id, optionId: foundOpt.id});
                }
    ...
    if (test.userFinished) {
        ...
        return (
            <div className="p-2 sm:p-4 pt-6 max-w-4xl mx-auto">
                ...
                <div
                    className="bg-green-50 border flex gap-2 border-green-300 text-green-700 p-4 rounded-xl mb-6">
                    <CheckCircle/>
                    Sizning natijangiz:{" "}
                ...
                <div className="sm:block">
                    <table
                        className="w-full border-collapse bg-white rounded-xl overflow-hidden shadow text-center">
