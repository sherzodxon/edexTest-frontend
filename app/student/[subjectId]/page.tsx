...
export default function SubjectTestsPage() {
    ...
    const [subject,
        setSubject] = useState < any > (null);
    const [results,
        setResults] = useState < any[] > ([]);
    const [loading,
        setLoading] = useState(true);
    const [error,
        setError] = useState < boolean > (false);
    const [notResult,
        setNotResults] = useState < boolean > (false)
    ...
    return (
        <div className="p-4 pt-6 sm:p-6 lg:p-8 space-y-6">
            <h1
                className="text-lg sm:text-xl lg:text-2xl font-bold flex gap-2 items-center text-gray-800">
                <Book className="w-5 h-5 sm:w-6 sm:h-6"/> {subject.name}
                ({subject.grade
                    ?.name})
            </h1>
                    ...
                    <div className="bg-white p-3 sm:p-4 border rounded-lg shadow-sm">
                        <ResponsiveContainer width="100%" height={300} className="sm:h-[300px]">
                            <LineChart data={results}>
                                ...
                                <Line type="monotone" dataKey="result" stroke="#27a55d" strokeWidth={3}/>
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                )
                        ...
                        <ul className="space-y-4">
                            {subject.tests
                                ?.map((test : any) => {
                                    ...
                                    const userTest = test.userTests
                                        ?.[0];
                                    const finished = userTest
                                        ?.finished;
                                    const score = userTest
                                        ?.score;

                                    return (
                                        <li
                                            key={test.id}
                                            className={`border p-3 sm:p-4 rounded-lg bg-white shadow-sm ${isActive
                                            ? "border-blue-500"
                                            : isExpired
                                            ...
                                            <div
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-500 flex flex-wrap items-center gap-2">
                                                        <AlarmClockMinus className="w-4 h-4"/> {start.toLocaleTimeString([], {
                                                            hour: "2-digit",
                                                            minute: "2-digit"
                                                        })}
                                                    ...
                                                    {finished && (
                                                        <p className="mt-1 text-sm text-green-600 flex gap-2 items-center">
                                                            <ListCheck/>
                                                            Siz bu testni {score ?? 0}
                                                            ball bilan yakunlagansiz
                                                        </p>
                                                    )}
                                                    {!finished && isExpired && (
                                                ...
                                                <div>
                                                    {finished
                                                        ? (
                                                            <button
                                                                onClick={() => router.push(`/student/tests/${test.id}/result`)}
