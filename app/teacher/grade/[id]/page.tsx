  ...
  <div className="border rounded-lg p-4 bg-white shadow-sm">
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-4">
      ...
      <Link
        ...
        className="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm sm:text-base transition-colors"
      >
    ...
    {tests.length === 0 ? (
      <p className="text-gray-500 text-center py-4">Bu fan uchun hali test yaratilmagan.</p>
    ) : (
      <ul className="space-y-3">
        {tests.map((t) => {
          ...
          let status = "yaratilgan"; // default
          if (start && now < start) status = "waiting";
          ...
          return (
            <li
              key={t.id}
              className="border p-3 rounded-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 hover:bg-gray-50 transition-shadow"
            >
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                   ...
                   {status === "waiting" && (
                    <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full border border-blue-200">Boshlanmagan</span>
                   )}
                   {status === "active" && (
                    <span className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full border border-green-200 animate-pulse">Davom etmoqda</span>
                   )}
                   {status === "finished" && (
                    <span className="text-[10px] bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full border border-gray-200">Tugagan</span>
                   )}
<span className="text-[10px] text-gray-500 flex items-center gap-1">
  <ListTodo className="w-3 h-3" />
  {(t.questionCount !== undefined ? t.questionCount : 0)} ta savol
</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                {status === "waiting" || !start ? (
                  <Link
                    href={`/teacher/tests/edit/${t.id}`}
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-amber-500 hover:bg-amber-600 text-white px-3 py-1.5 rounded-md text-sm transition-all shadow-sm"
                  >
                  ...
                  <button
                    disabled
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-gray-200 text-gray-500 px-3 py-1.5 rounded-md text-sm cursor-not-allowed border border-gray-300"
                  >
                  ...
                  <Link
                    ...
                    className="flex-1 sm:flex-none flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-sm transition-all shadow-sm"
                  >
