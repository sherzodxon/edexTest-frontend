...
export default function AdminSubjectsPage() {
    const [subjects,
        setSubjects] = useState < Subject[] > ([]);
    const [grades,
        setGrades] = useState < Grade[] > ([]);
    const [filtered,
        setFiltered] = useState < Subject[] > ([]);
    const [query,
        setQuery] = useState("");
    ...
    const [editingSubject,
        setEditingSubject] = useState < Subject | null > (null);
    ...
    useEffect(() => {
        ...
        setFiltered(subjects.filter((s) => s.name.toLowerCase().includes(q)));
    }, [query, subjects]);
        ...
        <div className="text-white">
            <div className="flex justify-between items-center mb-5">
                <h1 className="text-lg sm:text-2xl font-bold flex items-center gap-2">
                    <BookOpen className="text-[#FDAA48] w-5 h-5 sm:w-8 sm:h-8"/>
                    <span className="hidden sm:inline">Fanlar boshqaruvi</span>
                    <span className="text-orange-300">({subjects.length})</span>
                </h1>
                ...
                <button
                    ...
                    <span className="hidden sm:inline">Yangi fan qo'shish</span>
                </button>
                            ...
                            {filtered.map((s, i) => (
                                <div
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={() => handleEdit(s)} className="p-1">
                                            <Edit className="text-yellow-400 w-4 h-4 sm:w-5 sm:h-5 hover:text-yellow-300"/>
                                        </button>

                                        <button onClick={() => handleDelete(s.id)} className="p-1">
                                            <Trash2 className="text-red-500 w-4 h-4 sm:w-5 sm:h-5 hover:text-red-300"/>
                                        </button>
